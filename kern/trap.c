...
trap_init_percpu(void)
{
	...
	int cur_id = thiscpu->cpu_id;
	// Setup a TSS so that we get the right stack
	// when we trap to the kernel.
	thiscpu->cpu_ts.ts_esp0 = (uintptr_t)(percpu_kstacks[cur_id] + KSTKSIZE);
	thiscpu->cpu_ts.ts_ss0 = GD_KD;
	//cur_ts.ts_iomb = sizeof(struct Taskstate);

	// Initialize the TSS slot of the gdt.
	gdt[(GD_TSS0 >> 3) + cur_id] = SEG16(STS_T32A, (uint32_t) (&thiscpu->cpu_ts),
					sizeof(struct Taskstate) - 1, 0);
	gdt[(GD_TSS0 >> 3) + cur_id].sd_s = 0;

	// Load the TSS selector (like other segment selectors, the
	// bottom three bits are special; we leave them 0)
	ltr(GD_TSS0 + (cur_id << 3));
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if ((tf->tf_esp >= UXSTACKTOP - PGSIZE) && (tf->tf_esp < UXSTACKTOP)){
		utf =(void*) tf->tf_esp - sizeof(struct UTrapframe) - 4;//recursive case
	}else{
